...
export function detectParentPortalPlatform(): string {
  if (typeof window === 'undefined') return 'Portal padres (web - computador)';
  const isMobile = window.matchMedia('(max-width: 768px)').matches;
  return isMobile ? 'Portal padres (web - celular)' : 'Portal padres (web - computador)';
}
