...
import { Search, School as SchoolIcon, ArrowRight, User, LogOut, Package, Eye, EyeOff, HelpCircle, CreditCard, Smartphone, Building2, Shield, QrCode } from 'lucide-react';
...
export default function HomePageClient({ schools }: HomePageClientProps) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-surface-100 flex items-center justify-center"><div className="w-8 h-8 border-2 border-brand-600 border-t-transparent rounded-full animate-spin" /></div>}>
      <HomeContent schools={schools} />
    </Suspense>
  );
}

function HomeContent({ schools }: HomePageClientProps) {
  ...
  return (
    <div className="min-h-screen bg-surface-100 flex flex-col">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-1">
        ...
        {/* Payment Section - Wompi */}
        <div className="mb-12 bg-white rounded-xl border border-stone-200 overflow-hidden">
          <div className="bg-gradient-to-r from-stone-900 to-stone-800 text-white px-6 sm:px-8 py-5">
            <div className="flex items-center gap-3">
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2.5 mb-6">
              {[
                { name: 'Tarjetas', desc: 'Visa, Mastercard', icon: CreditCard },
                ...
                { name: 'Bancolombia', desc: 'QR o transferencia', icon: QrCode },
              ].map((method) => (
                <div
                  key={method.name}
                  className="flex flex-col items-center p-3 rounded-xl bg-surface-100 border border-stone-100 text-center"
                >
                  <method.icon className="w-5 h-5 text-brand-600 mb-1.5" />
                  <span className="text-xs font-semibold text-stone-800">{method.name}</span>
                  <span className="text-[10px] text-stone-400 hidden sm:block">{method.desc}</span>
                </div>
              ))}
            </div>

            {/* Trust indicators */}
            <div className="flex items-center justify-center gap-4 mb-6 py-3 bg-surface-100 rounded-lg border border-stone-100">
              ...
              <div className="text-[10px] text-stone-400 flex items-center gap-1">
                Procesado por <span className="font-semibold text-stone-500">Wompi</span>
              </div>
            </div>
