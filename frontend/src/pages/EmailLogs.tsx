...
export default function EmailLogs() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setRecentFailures(failuresData.items);
    } catch (err: any) {
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-stone-900">Log de Emails</h1>
            <p className="text-stone-500">Historial y estadisticas de emails enviados</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div>
              ...
              <select
                ...
                className="px-3 py-2 border border-stone-200 rounded-lg focus:ring-2 focus:ring-brand-400/30 focus:border-brand-500"
              >
            ...
            <div className="flex-1 min-w-[200px]">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-stone-200 rounded-lg focus:ring-2 focus:ring-brand-400/30 focus:border-brand-500"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-stone-500">Total Enviados</p>
                  <p className="text-3xl font-bold text-stone-900">{stats.total_sent}</p>
                </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-stone-500">Exitosos</p>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-stone-500">Fallidos</p>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-stone-500">Dev/Omitidos</p>
                  <p className="text-3xl font-bold text-stone-600">{stats.total_dev_skipped}</p>
                </div>
                <div className="p-3 bg-stone-100 rounded-full">
                  <Clock className="w-6 h-6 text-stone-600" />
                </div>
              </div>
              <p className="mt-2 text-sm text-stone-500">En modo desarrollo</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Daily Chart */}
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-stone-900 mb-4">Emails por Dia</h3>
            {chartData.length > 0 ? (
              <div className="h-64">
                <BarChart
                  data={chartData}
          ...
          {/* Type Breakdown */}
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-stone-900 mb-4">Por Tipo de Email</h3>
                ...
                {stats.by_type.map((type) => (
                  <div key={type.email_type} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded text-xs ${EMAIL_TYPE_COLORS[type.email_type]?.bg || 'bg-stone-100'} ${EMAIL_TYPE_COLORS[type.email_type]?.text || 'text-stone-700'}`}>
                        {type.email_type_label}
                      </span>
                    </div>
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-stone-600">{type.total} total</span>
            ...
            <div className="space-y-2">
              {recentFailures.map((log) => (
                <div key={log.id} className="flex items-center justify-between bg-white rounded p-3">
                  <div>
                    <span className="font-medium text-stone-900">{log.recipient_email}</span>
                    <span className="text-stone-500 mx-2">-</span>
                    <span className="text-stone-600">{log.email_type_label}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-sm text-stone-500">{formatDateTime(log.sent_at)}</span>
                    <button
                    ...
                    {logs.map((log) => (
                      <tr key={log.id} className="hover:bg-stone-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 rounded text-xs ${EMAIL_TYPE_COLORS[log.email_type]?.bg || 'bg-stone-100'} ${EMAIL_TYPE_COLORS[log.email_type]?.text || 'text-stone-700'}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            ...
                            {log.recipient_name && (
                              <p className="text-xs text-stone-500">{log.recipient_name}</p>
                            )}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="px-6 py-4 border-t border-stone-200 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 rounded-lg border border-stone-200 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-stone-50"
                    >
            ...
            <div className="flex items-center justify-center min-h-screen px-4">
              ...
              <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-sm font-medium text-stone-500">Tipo</label>
                    <p className="text-stone-900">{selectedLog.email_type_label}</p>
                  </div>

                  <div>
                    <label className="text-sm font-medium text-stone-500">Destinatario</label>
                    <p className="text-stone-900">{selectedLog.recipient_email}</p>
                    {selectedLog.recipient_name && (
                      <p className="text-sm text-stone-500">{selectedLog.recipient_name}</p>
                    )}
                  </div>

                  <div>
                    <label className="text-sm font-medium text-stone-500">Asunto</label>
                    <p className="text-stone-900">{selectedLog.subject}</p>
                  </div>

                  <div>
                    <label className="text-sm font-medium text-stone-500">Estado</label>
                    <p>
                  ...
                  <div>
                    <label className="text-sm font-medium text-stone-500">Fecha de Envio</label>
                    <p className="text-stone-900">{formatDateTime(selectedLog.sent_at)}</p>
                  </div>

                  {selectedLog.reference_code && (
                    <div>
                      <label className="text-sm font-medium text-stone-500">Referencia</label>
                      <p className="text-stone-900">{selectedLog.reference_code}</p>
                    </div>
                  )}

                  {selectedLog.client_name && (
                    <div>
                      <label className="text-sm font-medium text-stone-500">Cliente</label>
                      <p className="text-stone-900">{selectedLog.client_name}</p>
                    </div>
                  )}

                  {selectedLog.triggered_by_name && (
                    <div>
                      <label className="text-sm font-medium text-stone-500">Disparado por</label>
                      <p className="text-stone-900">{selectedLog.triggered_by_name}</p>
                    </div>
                  )}
