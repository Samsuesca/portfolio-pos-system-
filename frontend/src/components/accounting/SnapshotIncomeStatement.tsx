  ...
  return (
    <div className="space-y-4">
        ...
        {showBreakdown && bySchool.length > 0 && (
          <div className="ml-6 mt-1 mb-2 space-y-1">
            {bySchool.map((school, i) => (
              <div key={str(school.school_id) || i} className="flex justify-between text-sm text-stone-500">
      ...
      <div className="border-b pb-4">
        ...
        <p className="text-xs text-stone-500">Margen Bruto: {num(data.gross_margin_percent).toFixed(1)}%</p>
      </div>
      ...
      <div className="border-b pb-4">
        ...
        <p className="text-xs text-stone-500">Margen Operacional: {num(data.operating_margin_percent).toFixed(1)}%</p>
      </div>
      ...
      <div className="border-b pb-4">
        ...
        {otherExpenses.map((cat, i) => (
          <div key={str(cat.category) || i} className="flex justify-between text-sm py-1">
            <span className="text-stone-600">(-) {getCategoryLabel(str(cat.category))}</span>
            <span className="text-red-500">{formatCurrency(num(cat.total))}</span>
          </div>
        ))}
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4">
        ...
        <p className="text-sm text-stone-500 mt-1">Margen Neto: {num(data.net_margin_percent).toFixed(1)}%</p>
      </div>
    </div>
  );
};
