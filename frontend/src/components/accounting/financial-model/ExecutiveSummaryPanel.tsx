...
function ChangeIndicator({ value }: { value: number | null }) {
  if (value === null || value === undefined) return <span className="text-stone-400 text-xs">N/A</span>;
  ...
  return <span className="text-stone-400 text-xs flex items-center gap-0.5"><Minus className="w-3 h-3" />0%</span>;
}
  ...
  if (!data) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-8 text-center">
        ...
        <p className="text-stone-500">No hay datos para el resumen ejecutivo</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between print:hidden">
        <h3 className="text-lg font-semibold text-stone-800">Resumen Ejecutivo</h3>
        <button
          onClick={() => window.print()}
          className="flex items-center gap-2 bg-stone-100 hover:bg-stone-200 text-stone-700 px-4 py-2 rounded-lg text-sm transition-colors"
        >
      ...
      {/* Printable content */}
      <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-6 print:shadow-none print:border-0 print:p-0">
        {/* Header */}
        <div className="text-center mb-6 pb-4 border-b border-stone-200">
          <h2 className="text-xl font-bold text-stone-800">Uniformes Consuelo Ríos</h2>
          <p className="text-stone-500 text-sm mt-1">Resumen Financiero — {data.period_label}</p>
        </div>

        {/* Banner de mes parcial: las cifras del mes en curso no son
            comparables al mes completo. Sin este aviso el usuario lee
            "-92.7%" alarmista cuando solo van pocos días del mes. */}
        {data.period_warning && (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-brand-50 rounded-lg p-4">
            <p className="text-xs text-brand-600 uppercase font-medium">Ingresos</p>
            <p className="text-lg font-bold text-brand-700 mt-1">{formatMoney(data.revenue)}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Top 3 schools */}
          <div>
            <h4 className="text-sm font-semibold text-stone-700 mb-3">Top 3 Colegios por Ingresos</h4>
              ...
              {data.top_schools.map((s, i) => (
                <div key={i} className="flex items-center justify-between bg-stone-50 rounded-lg px-3 py-2">
                  ...
                  <div className="text-right">
                    <span className="text-sm font-semibold text-stone-800">{formatMoney(s.amount)}</span>
                    <span className="text-xs text-stone-500 ml-2">({Number(s.percentage).toFixed(1)}%)</span>
                  </div>
                </div>
              ))}
          ...
          <div>
            <h4 className="text-sm font-semibold text-stone-700 mb-3">Top 3 Categorías de Gasto</h4>
            <div className="space-y-2">
              {data.top_expense_categories.map((c, i) => (
                <div key={i} className="flex items-center justify-between bg-stone-50 rounded-lg px-3 py-2">
                  ...
                  <div className="text-right">
                    <span className="text-sm font-semibold text-stone-800">{formatMoney(c.amount)}</span>
                    <span className="text-xs text-stone-500 ml-2">({Number(c.percentage).toFixed(1)}%)</span>
                  </div>
                </div>
              ))}
        ...
        {data.kpi_snapshot.length > 0 && (
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-stone-700 mb-3">Indicadores Clave</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {data.kpi_snapshot.map((kpi) => (
                <div key={kpi.key} className="bg-stone-50 rounded-lg p-3 text-center">
                  ...
                  <p className="text-sm font-bold text-stone-800">{kpi.formatted_value}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {data.active_alerts.length > 0 && (
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-stone-700 mb-3">Alertas Activas ({data.active_alerts.length})</h4>
            <div className="space-y-2">
              {data.active_alerts.slice(0, 5).map((alert, i) => {
                const Icon = ALERT_ICON[alert.severity] || Info;
                const color = ALERT_COLOR[alert.severity] || 'text-stone-600';
                return (
                  <div key={i} className="flex items-center gap-2 text-sm">
                    <Icon className={`w-4 h-4 ${color} shrink-0`} />
                    <span className="text-stone-700">{alert.title}: {alert.message.substring(0, 80)}{alert.message.length > 80 ? '...' : ''}</span>
                  </div>
                );
              })}
            </div>
        ...
        {data.forecast_summary && (
          <div className="bg-stone-50 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-stone-700 mb-1">Proyección (3 meses)</h4>
            <p className="text-sm text-stone-600">{data.forecast_summary}</p>
          </div>
        )}
