...
const STATUS_STYLES = {
  within: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200',
  near_limit: 'bg-amber-50 text-amber-700 ring-1 ring-amber-200',
  over: 'bg-red-50 text-red-700 ring-1 ring-red-200',
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-stone-800">Presupuesto vs Real</h3>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white px-4 py-2 rounded-lg text-sm transition-colors"
        >
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-6">
          <h4 className="font-medium text-stone-800 mb-4">Crear Presupuesto</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Tipo de Período</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Monto Presupuestado</label>
              <input
                type="number"
            ...
            <div className="flex items-end">
              <button
                ...
                disabled={submitting}
                className="bg-brand-500 hover:bg-brand-600 text-white px-6 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
              >
      ...
      {/* Empty state cuando no hay budgets ni comparativo. Antes la
          pantalla quedaba con solo el header y el botón, dando sensación
          de "rota" (QA P2-04 2026-05-04). */}
      {!showForm && budgets.length === 0 && (!budgetVsActual || budgetVsActual.items.length === 0) && (
        <div className="text-center py-12 bg-stone-50 rounded-xl border border-dashed border-stone-300">
          ...
          <p className="text-stone-600 text-sm font-medium">Aún no has definido presupuestos</p>
          <p className="text-stone-500 text-xs mt-1 max-w-md mx-auto">
            Define metas mensuales o trimestrales por categoría (renta, nómina, marketing) para
            ver cómo va tu ejecución vs lo planeado. Empieza con el botón <strong>Nuevo Presupuesto</strong>.
          </p>
          <button
            onClick={() => setShowForm(true)}
            className="mt-4 inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white px-4 py-2 rounded-lg text-sm transition-colors"
          >
            <Plus className="w-4 h-4" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-stone-50 border-b border-stone-200">
                  ...
                  <th className="text-right px-4 py-3 font-medium text-stone-600">Presupuesto</th>
                  <th className="text-right px-4 py-3 font-medium text-stone-600">Real</th>
                  <th className="text-right px-4 py-3 font-medium text-stone-600">Variación</th>
                  <th className="text-right px-4 py-3 font-medium text-stone-600">%</th>
                  <th className="text-center px-4 py-3 font-medium text-stone-600">Estado</th>
                </tr>
              </thead>
              <tbody>
                {budgetVsActual.items.map((item) => (
                  <tr key={item.category} className="border-b border-stone-100 hover:bg-stone-50">
                    <td className="px-4 py-3 font-medium text-stone-800">{item.category_label}</td>
      ...
      {budgets.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-6">
          <h4 className="font-medium text-stone-800 mb-4">Presupuestos Creados</h4>
          <div className="space-y-2">
            {budgets.map((b) => (
              <div key={b.id} className="flex items-center justify-between bg-stone-50 rounded-lg px-4 py-3">
                <div>
                  <span className="font-medium text-stone-800">{b.category}</span>
                  <span className="text-stone-500 text-sm ml-2">
                    {b.period_start} — {b.period_end} ({b.period_type})
                  </span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="font-semibold text-stone-800">{formatMoney(b.budgeted_amount)}</span>
                  <button
      ...
      {!budgetVsActual && budgets.length === 0 && !showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-8 text-center">
          ...
          <p className="text-stone-500">No hay presupuestos creados</p>
          <p className="text-stone-400 text-sm mt-1">Cree un presupuesto para comparar con los gastos reales</p>
        </div>
      )}
    </div>
  );
