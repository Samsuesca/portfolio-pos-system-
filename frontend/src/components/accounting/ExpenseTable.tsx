...
const ExpenseTable: React.FC<ExpenseTableProps> = ({
    ...
    return (
      <div className="bg-white rounded-xl border border-stone-200 p-12">
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-stone-500">Cargando gastos...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
      {/* Table Header */}
      <div className="hidden md:grid md:grid-cols-12 gap-4 px-5 py-3 bg-stone-50 border-b border-stone-200 text-xs font-semibold text-stone-600 uppercase tracking-wide">
      ...
      <div className="divide-y divide-stone-100">
        {expenses.map((expense) => {
          ...
          return (
            <div key={expense.id} className={`transition ${isExpanded ? 'bg-stone-50' : 'hover:bg-stone-50'}`}>
              {/* Row */}
              <div
              ...
              >
                ...
                <div className="col-span-12 md:col-span-4 order-last md:order-none">
                  ...
                  {expense.vendor_name && (
                    <p className="text-sm text-stone-500 truncate">{expense.vendor_name}</p>
                  )}
                </div>
                ...
                <div className="col-span-4 md:col-span-2">
                  {expense.is_paid ? (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-full bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200">
                ...
                <div className="col-span-2 md:col-span-1 text-right">
                  {isExpanded ? (
                    <ChevronUp className="w-5 h-5 text-stone-400 inline" />
                  ) : (
                    <ChevronDown className="w-5 h-5 text-stone-400 inline" />
                  )}
                </div>
              </div>
              ...
              {/* Expanded Details */}
              {isExpanded && (
                <div className="px-5 pb-5 pt-2 border-t border-stone-200 bg-white">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="space-y-2 text-sm">
                      {expense.due_date && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-stone-600">Vencimiento:</span>
                      ...
                      {expense.is_paid && expense.payment_account_name && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-stone-600">Cuenta:</span>
                          <span className="font-medium text-stone-900">{expense.payment_account_name}</span>
                        </div>
                      )}
                      {expense.is_paid && expense.payment_method && (
                        <div className="flex items-center gap-2">
                          <CreditCard className="w-4 h-4 text-stone-400" />
                          <span className="text-stone-600">Metodo:</span>
                          <span className="font-medium text-stone-900">
                    ...
                    {expense.notes && (
                      <div className="text-sm">
                        ...
                        <p className="text-stone-900 mt-1">{expense.notes}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-3 border-t border-stone-100">
                    ...
                    {!expense.is_paid && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 rounded-lg text-sm font-medium hover:bg-green-200 transition"
                      >
                    ...
                    {!expense.is_paid && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 bg-red-50 text-red-700 ring-1 ring-red-200 rounded-lg text-sm font-medium hover:bg-red-200 transition"
                      >
      ...
      {hasMore && (
        <div className="px-5 py-4 border-t border-stone-200 bg-stone-50">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 text-brand-600 hover:text-brand-700 font-medium disabled:opacity-50"
          >
            {loadingMore ? (
              <>
      ...
      {/* Footer Summary */}
      <div className="px-5 py-4 border-t border-stone-200 bg-stone-50 flex justify-between items-center">
