    ...
    <>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white rounded-lg shadow-sm border border-stone-200 p-4">
            <p className="text-xs font-medium text-stone-500">Total por Pagar</p>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-stone-200 p-4">
            ...
            <p className="text-xs text-stone-400">{summary.payables_count} cuenta(s)</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-stone-200 p-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-stone-200 p-4">
            <p className="text-xs font-medium text-stone-500">Posicion Neta</p>
            <p className={`text-xl font-bold ${summary.net_position >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {formatCurrency(summary.net_position)}
            </p>
            <p className="text-xs text-stone-400">Por cobrar - Por pagar</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-stone-200">
        <div className="px-6 py-4 border-b border-stone-200 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-stone-800">Cuentas por Pagar</h3>
          <button
        ...
        <div className="divide-y divide-stone-100">
          {payablesList.length === 0 ? (
            ...
            payablesList.map((item) => (
              <div key={item.id} className="px-6 py-4 hover:bg-stone-50 transition-colors">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="font-semibold text-stone-800">{formatCurrency(item.amount)}</p>
                      {item.amount_paid > 0 && (
                        <p className="text-xs text-stone-500">Pagado: {formatCurrency(item.amount_paid)}</p>
                      )}
                      {item.balance > 0 && (
                    ...
                    {!item.is_paid && (
                      <button
                        ...
                        className="px-3 py-1 bg-red-50 text-red-700 ring-1 ring-red-200 rounded-lg text-sm hover:bg-red-200 transition-colors"
                      >
                        Pagar
                      </button>
                    )}
