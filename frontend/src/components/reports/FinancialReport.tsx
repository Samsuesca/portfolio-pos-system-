  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-3 text-stone-600">Cargando datos financieros...</span>
      </div>
    );
  }
    ...
    <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-stone-500">Ingresos</span>
            </div>
            <h3 className="text-2xl font-bold text-green-600">
              {formatCurrency(cashFlow.total_income)}
            </h3>
            <p className="text-sm text-stone-600 mt-1">Total del periodo</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-stone-500">Gastos</span>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <div className={`p-2 rounded-lg ${cashFlow.net_flow >= 0 ? 'bg-brand-100' : 'bg-orange-100'}`}>
                <Wallet className={`w-6 h-6 ${cashFlow.net_flow >= 0 ? 'text-brand-600' : 'text-orange-600'}`} />
              </div>
              <span className="text-xs text-stone-500">Flujo Neto</span>
            </div>
            <h3 className={`text-2xl font-bold ${cashFlow.net_flow >= 0 ? 'text-brand-600' : 'text-orange-600'}`}>
              {cashFlow.net_flow >= 0 ? '+' : ''}{formatCurrency(cashFlow.net_flow)}
            </h3>
            <p className="text-sm text-stone-600 mt-1">Ingresos - Gastos</p>
          </div>
        </div>
      )}
      ...
      {cashFlow && cashFlow.periods.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-stone-800 flex items-center mb-4">
            ...
            {cashFlow.periods.slice(0, 10).map((period) => {
              ...
              return (
                <div key={period.period} className="flex items-center gap-4">
                  ...
                  <div className={`w-24 text-right text-sm font-medium ${period.net >= 0 ? 'text-brand-600' : 'text-orange-600'}`}>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="p-6 border-b border-stone-200">
            <h2 className="text-lg font-semibold text-stone-800 flex items-center">
              <Receipt className="w-5 h-5 mr-2 text-brand-600" />
              Ultimas Transacciones
            </h2>
            <p className="text-sm text-stone-500 mt-1">{dateRangeLabel || 'Periodo seleccionado'}</p>
          </div>
                  ...
                  {transactions.slice(0, 20).map((tx) => (
                    <tr key={tx.id} className="hover:bg-stone-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="text-sm text-stone-900 line-clamp-1">
                          {tx.description}
                        </div>
                        {tx.school_name && (
                          <div className="text-xs text-stone-500">{tx.school_name}</div>
                        )}
                      </td>
                      <td className={`px-4 py-3 text-right text-sm font-medium whitespace-nowrap ${
                        tx.type === 'income' ? 'text-green-600' : tx.type === 'expense' ? 'text-red-600' : 'text-stone-600'
                      }`}>
              ...
              {expensesByCategory.map((cat) => (
                <div key={cat.category}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-stone-700">{cat.category_label}</span>
                    <span className="text-sm text-stone-600">{formatCurrency(cat.total_amount)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-stone-500 w-12 text-right">{Number(cat.percentage).toFixed(1)}%</span>
                  </div>
