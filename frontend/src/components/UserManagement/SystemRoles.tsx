    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        {SYSTEM_ROLES.map((sysRole) => (
          <div
            key={sysRole.role}
            className="p-4 bg-stone-50 border border-stone-200 rounded-lg"
          >
            <div className="flex items-center justify-between mb-2">
              <RoleBadge role={sysRole.role} size="sm" />
              <span className="text-xs text-stone-500">{sysRole.permissions} permisos</span>
            </div>
            <p className="text-xs text-stone-600">{sysRole.description}</p>
          </div>
        ))}
      </div>
    </div>
