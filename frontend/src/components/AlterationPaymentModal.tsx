...
const PAYMENT_METHODS = [
  ...
  { value: 'transfer', label: 'Transferencia', icon: Building2, color: 'text-brand-600' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const paymentData: AlterationPaymentCreate = {
        ...
        ...(formData.payment_method === 'cash' && formData.amount_received
        ...
        <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl">
          ...
          <div className="px-6 py-4 bg-stone-50 border-b border-stone-100">
            ...
            <div className="flex items-center justify-between mt-1 text-sm">
              <span className="text-stone-500">Total: {formatCurrency(alteration.cost)}</span>
              <span className="text-stone-500">Pagado: {formatCurrency(alteration.amount_paid)}</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  step="100"
                  className="w-full pl-8 pr-3 py-3 text-lg border border-stone-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                />
              </div>
              {/* Quick amount buttons */}
              <div className="flex gap-2 mt-2">
                <button
                  ...
                  className="flex-1 py-1.5 text-xs bg-stone-100 text-stone-600 rounded hover:bg-stone-200 transition-colors"
                >
                  50%
                </button>
                <button
                ...
                <button
                  ...
                  className="flex-1 py-1.5 text-xs bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 rounded hover:bg-green-200 transition-colors"
                >
                  Total
                </button>
              </div>
                  ...
                  return (
                    <button
                      ...
                      onClick={() => setFormData(prev => ({
                        ...
                        ...(method.value !== 'cash' ? { amount_received: '' } : {}),
                      }))}
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium ${isSelected ? 'text-brand-700' : 'text-stone-700'}`}>
                        {method.label}
                      </span>
                    </button>
                  );
            ...
            {formData.payment_method === 'cash' && (
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={formData.amount ? `Min: $${parseFloat(formData.amount).toLocaleString()}` : 'Opcional'}
                  ...
                  {formData.amount_received && formData.amount && parseFloat(formData.amount_received) >= parseFloat(formData.amount) && (
                    <div className="text-right min-w-[100px]">
                      <span className="text-xs text-amber-700">Devueltas:</span>
                      <p className="text-xl font-bold text-amber-800">
                        ${(parseFloat(formData.amount_received) - parseFloat(formData.amount)).toLocaleString()}
                      </p>
                    </div>
                  )}
                </div>
                ...
                {formData.amount_received && formData.amount && parseFloat(formData.amount_received) < parseFloat(formData.amount) && (
                  <p className="text-xs text-red-600 mt-2">
                    Monto insuficiente. Minimo: ${parseFloat(formData.amount).toLocaleString()}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg">
              ...
              />
              <label htmlFor="apply_accounting" className="text-sm text-stone-700 cursor-pointer">
