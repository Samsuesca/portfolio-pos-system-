              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {displayData.reason && (
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    ...
                    <p className="text-sm text-gray-900 mt-1">{displayData.reason}</p>
                  </div>
                )}
                ...
                <div className="flex flex-col md:flex-row gap-4 items-stretch">
                  ...
                  <div className="flex-1 bg-white rounded-lg p-4 border border-gray-200">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="pt-2 border-t border-gray-100 mt-2">
                        <p className="text-xs text-gray-500">Cantidad: <span className="font-semibold text-gray-900">{displayData.returned_quantity}</span></p>
                        {displayData.original_unit_price && (
                          <p className="text-xs text-gray-500">
                            P. Unitario: <span className="font-semibold text-gray-900">{formatCurrency(displayData.original_unit_price)}</span>
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {displayData.new_product_code && (
                    <div className="flex items-center justify-center md:py-0 py-2">
                      <ArrowRight className="w-8 h-8 text-blue-400 transform md:rotate-0 rotate-90" />
                    </div>
                  )}
                  ...
                  {displayData.new_product_code ? (
                    <div className="flex-1 bg-white rounded-lg p-4 border border-gray-200">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="pt-2 border-t border-gray-100 mt-2">
                          <p className="text-xs text-gray-500">Cantidad: <span className="font-semibold text-gray-900">{displayData.new_quantity}</span></p>
                          {displayData.new_unit_price && (
                            <p className="text-xs text-gray-500">
                              P. Unitario: <span className="font-semibold text-gray-900">{formatCurrency(displayData.new_unit_price)}</span>
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex-1 bg-gray-100 rounded-lg p-4 border border-gray-200 flex items-center justify-center">
                      <span className="text-gray-500 text-sm italic">Sin producto de reemplazo (devolucion)</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {details && (
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  {details.transactions && details.transactions.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-xs font-semibold text-gray-600 mb-2">Transacciones Contables:</h4>
                        ...
                        {details.transactions.map((t) => (
                          <div key={t.id} className="flex items-center justify-between bg-white rounded px-3 py-2 text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-700">{t.description || t.type}</span>
                            </div>
                            <div className="flex items-center gap-4">
                              <span className={`font-semibold ${Number(t.amount) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                {formatCurrency(Number(t.amount))}
                              </span>
                              <span className="text-xs text-gray-400">{formatDate(t.transaction_date)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {details.inventory_movements && details.inventory_movements.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-xs font-semibold text-gray-600 mb-2">Movimientos de Inventario:</h4>
                      <div className="space-y-2">
                        {details.inventory_movements.map((m) => (
                          <div key={m.id} className="flex items-center justify-between bg-white rounded px-3 py-2 text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-mono text-xs text-gray-600">{m.product_code}</span>
                              <span className="text-gray-700">{m.product_name}</span>
                            </div>
                            <div className="flex items-center gap-2">
                  ...
                  {details.associated_order && (
                    <div>
                      <h4 className="text-xs font-semibold text-gray-600 mb-2">Pedido Asociado:</h4>
                      <button
                        onClick={() => {
                          onClose();
                          navigate(`/orders/${details.associated_order!.id}`);
                        }}
                        className="flex items-center gap-2 bg-white rounded px-3 py-2 text-sm hover:bg-gray-50 transition-colors w-full"
                      >
                        <ShoppingCart className="w-4 h-4 text-blue-500" />
                        <span className="font-medium text-blue-600">{details.associated_order.code}</span>
                        <span className="text-gray-500">- {details.associated_order.status}</span>
